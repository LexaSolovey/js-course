...
function doRectanglesOverlap(rect1, rect2) {
    if(rect1.left > rect2.width + rect2.left || rect1.left + rect1.width < rect2.left||
        rect1.top > rect2.height + rect2.top || rect1.top + rect1.height< rect2.top) return false
    else return true;
}
...
function isInsideCircle(circle, point) {
    if( Math.pow(circle.center.x - point.x, 2) + Math.pow(circle.center.y - point.y,2)< Math.pow(circle.radius,2)) return true
    else return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let finalStr = "";
  if(isStartIncluded === true) finalStr += "\[";
  else finalStr += "\(";
  if(a > b){
    finalStr += b +", ";
    finalStr += a;
  ...
  if(isEndIncluded === true) finalStr += "\]";
  else finalStr += "\)";
  return finalStr;
}
...
function reverseInteger(num) {
    ...
    let numbStr = numArr.join('');
    return Number(numbStr);
}
